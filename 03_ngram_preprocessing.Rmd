...
## Split Fukk Datasets to Training and Test Sets
...
overall_sampling_rate=0.1 # Blended sampling rate from training data set
set.seed(0)
...
## Prepare a Small Training Data Set To Support Development of Predictive Model

```{r}
target_path='intermediate/pre-processed-filtered/'
dir.create(target_path)

write_rds(x=filter(word_sample, rank<=10000), path=paste0(target_path,'unigrams_filtered.rds'), compress="gz")
write_rds(x=filter(bigram_sample, rank<=10000), path=paste0(target_path,'bigrams_filtered.rds'), compress="gz")
write_rds(x=filter(trigram_sample, rank<=10000), path=paste0(target_path,'trigrams_filtered.rds'), compress="gz")
write_rds(x=filter(quadrigram_sample, rank<=10000), path=paste0(target_path,'quadrigrams_filtered.rds'), compress="gz")

message('\nFiltered n-grams saved')
```
